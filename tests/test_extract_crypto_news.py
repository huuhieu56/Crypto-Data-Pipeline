...
class TestFetchArticles:
    ...
    @patch("scripts.extract_modules.extract_crypto_news.requests.get")
    @patch("scripts.extract_modules.extract_crypto_news.GNEWS_API_KEY", "test-key")
    def test_happy_path_returns_articles(self, mock_get):
    ...
    @patch("scripts.extract_modules.extract_crypto_news.GNEWS_API_KEY", "")
    ...
    @patch("scripts.extract_modules.extract_crypto_news.requests.get")
    @patch("scripts.extract_modules.extract_crypto_news.GNEWS_API_KEY", "test-key")
    def test_api_http_error_raises_extract_error(self, mock_get):
    ...
    @patch("scripts.extract_modules.extract_crypto_news.requests.get")
    @patch("scripts.extract_modules.extract_crypto_news.GNEWS_API_KEY", "test-key")
    def test_network_timeout_raises_extract_error(self, mock_get):
...
class TestExtractCryptoNews:
    ...
    def _setup(self):
        """Setup chung: mock append_to_partition."""
        mod = sys.modules["scripts.extract_modules.extract_crypto_news"]

        self.mock_append = MagicMock()
        self._orig_append = mod.append_to_partition
        mod.append_to_partition = self.mock_append
        yield
        mod.append_to_partition = self._orig_append

    # --- Happy Path ---
    @patch("scripts.extract_modules.extract_crypto_news._fetch_articles")
    def test_happy_path_returns_counts(self, mock_fetch):
        """Chạy thành công → trả về dict counts."""
        mock_fetch.return_value = SAMPLE_GNEWS_RESPONSE["articles"]
    ...
    @patch("scripts.extract_modules.extract_crypto_news._fetch_articles")
    def test_writes_to_correct_minio_path(self, mock_fetch):
    ...
    @patch("scripts.extract_modules.extract_crypto_news._fetch_articles")
    def test_dedup_col_is_article_id(self, mock_fetch):
    ...
    @patch("scripts.extract_modules.extract_crypto_news._fetch_articles")
    def test_multiple_queries(self, mock_fetch):
        """Nhiều queries → fetch nhiều lần."""
        mock_fetch.return_value = SAMPLE_GNEWS_RESPONSE["articles"]

        result = extract_crypto_news(
    ...
    # --- Sad Path ---
    @patch("scripts.extract_modules.extract_crypto_news._fetch_articles")
    def test_no_articles_found(self, mock_fetch):
    ...
    @patch("scripts.extract_modules.extract_crypto_news._fetch_articles")
    def test_api_key_missing_raises(self, mock_fetch):
        """API key thiếu → ExtractError propagated."""
        mock_fetch.side_effect = ExtractError("GNews API key not configured")

        with pytest.raises(ExtractError):
            extract_crypto_news(queries=["bitcoin"])

    @patch("scripts.extract_modules.extract_crypto_news._fetch_articles")
    def test_one_query_fails_others_continue(self, mock_fetch):
    ...
    # --- Edge Cases ---
    @patch("scripts.extract_modules.extract_crypto_news._fetch_articles")
    def test_default_queries_from_config(self, mock_fetch):
        """Không truyền queries → dùng GNEWS_SEARCH_QUERIES từ config."""
        mod = sys.modules["scripts.extract_modules.extract_crypto_news"]

        mock_fetch.return_value = SAMPLE_GNEWS_RESPONSE["articles"]
        orig = mod.GNEWS_SEARCH_QUERIES
        mod.GNEWS_SEARCH_QUERIES = ["cryptocurrency OR bitcoin"]
        try:
            extract_crypto_news()
            ...
            mod.GNEWS_SEARCH_QUERIES = orig

    @patch("scripts.extract_modules.extract_crypto_news._fetch_articles")
    def test_dataframe_columns_correct(self, mock_fetch):
