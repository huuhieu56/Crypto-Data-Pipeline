...
class TestLoadNews:
    """Tests cho load_news() — thin wrapper around load_table."""
    ...
    def _setup(self):
        ...
        self.orig_load_table = self._mod.load_table
        yield
        self._mod.load_table = self.orig_load_table

    def test_calls_load_table_with_correct_args(self):
        """load_news() gọi load_table với đúng args (extracted_at watermark)."""
        mock_load_table = MagicMock()
        self._mod.load_table = mock_load_table

        load_news()
        mock_load_table.assert_called_once_with(
            None, "crypto-processed", "crypto_news", "extracted_at",
    ...
    def test_propagates_load_error(self):
        """LoadError từ load_table được propagate."""
